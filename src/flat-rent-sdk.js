...
export class FlatRentSdk {
     ...
     * Search for flats.
     * 
     * @param {SearchFilter} parameters Search parameters
    ...
    search(parameters) {
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!(parameters.checkInDate[1] instanceof Date) || !(parameters.checkOutDate[1] instanceof Date)) {
                    throw new Error(`Passed invalid check-in or check-out date - from "${parameters.checkInDate[1]}" to "${parameters.checkOutDate[1]}".`)
                }
                this._assertDatesAreCorrect(parameters.checkInDate[1], parameters.checkOutDate[1])
                ...
                const dateRange = this._generateDateRange(parameters.checkInDate[1], parameters.checkOutDate[1])
                flats = flats.filter((flat) => {
